...
class QuadraticRegularizer(Solver):
  ...
  def run_one_iteration(self):
    ...
    if self.initialize:
      self.weights = -1 * self.eta * self.z_t
      self.initialize = False
    else:
      self.weights = -1 * self.eta * self.cum_z_t

    # Normalize new decision vector
    self.p_t = self.weights/np.sum(self.weights)
