...
class Solver(object):
  def __init__(self, bandit):
    ...
    assert isinstance(bandit, epsGreedy_BetaBandit) or isinstance(bandit, EXP3_BetaBandit)
  ...
  def update_regret(self, ep_num, itr, arm, bandit_dist_vector):
    ...
    # Calculate mean cumulative regret over timesteps
    if ep_num == self.epochs - 1 and itr == self.T - 1:
      itrs = range(1, self.T+1)
      self.regret_mean = np.mean(self.regret.copy(), 0)/itrs
